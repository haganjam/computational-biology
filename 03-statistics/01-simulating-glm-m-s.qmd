---
title: "Simulating Generalised Linear Mixed Models"
...
and your statistics skills? That solution is simulation and we love it here at DataWise.

## Example N: Biodiversity in food forests

I was present at a lively debate recently about the correct way to analyse an experiment
that examined how the [*food-forest*](https://projectfoodforest.org/what-is-a-food-forest/) land-use type affects biodiversity. Specifically, the
...
were two plots from the two groups i.e. food forest and agricultural field (there were actually four groups but, for simplicity, we will use two for this example). Within each of plots at each of the
...
following structure (where B - biodiversity, T - treatment and L - location) as represented in the classic tilde notation: 
...
equation for now if you are not so inclined. However, I wrote it here because we will want to come back to all the different
parameters in this model.

$$
B_{i} \sim Normal(\mu_{i}, \sigma_{residual})
$$
$$
\mu_{i} = \alpha_{L[i]} + \beta_1(T_{i})
$$
$$
\alpha_{j} \sim Normal(\mu_{global}, \sigma_{global}) \text{, for location (L) j = 1,} \dots \text{,J}
$$
...
($\mu_{i}$). The observed biodiversity values ($B_{i}$) are normally distributed around these mean
values ($\mu_{i}$) for each location-group combination with some residual standard deviation
($\sigma_{residual}$).
...
We are going to do these simulations in [R](https://www.r-project.org) so feel free to follow along with the code. Before we start, we are going to load a few libraries that we will use for these simulations and the analysis.

```{r}
#| output: false
#| warning: false

# function to check if a package is installed and then install it
...
```{r}
#| label: fig-1
#| fig-cap: "Base-level biodiversity at each location in the control group (i.e. the agricultural field)."
...
dist_global <- rnorm(n = 1000000, mean = mu_global, sd = sigma_global)
...
global distribution (i.e. @fig-1). These 30 samples are then the 30 base-level biodiversity values for the
agricultural field plot (i.e. control plot) at each location ($\alpha_j$). We can visualise these
30 location mean values as red vertical lines on the global distribution (@fig-2):

```{r}
#| label: fig-2
#| fig-cap: "Samples (red, vertical lines) from the distribution of base-level biodiversity at each location in the control group (i.e. the agricultural field)."
...
($\alpha_j$) (@fig-2). Let's visualise this in a different way that is more intuitive. Specifically, we plot
each mean value on the y-axis and the x-axis simply indices the 30 locations. Moreover, we plot the
global mean as a red, dashed, horizontal line.

```{r}
#| label: fig-3
#| fig-cap: "The base-level biodiversity values for the agricultural fields in the 30 sampled locations. The red, dashed horizontal line is the global mean of the distribution."
...
in food forests will be greater than in the agricultural fields by 5 units. We
then add this treatment effect to all locations which gives us the mean biodiversity value at the 30
locations for the food forest (@fig-4).

```{r}
#| label: fig-4
#| fig-cap: "The base-level biodiversity values in the agricultural fields and the biodiversity values in the food forests in the 30 sampled locations."
...
# visualise the results as a location scatterplot
ggplot(data = dat_mu,
       mapping = aes(x = location, y = mu_i, colour = treatment)) +
  geom_point() +
  geom_hline(yintercept = mu_global, colour = "red", linetype = "dashed") +
...
model (we hope) (@fig-4). The last thing we need to do is simulate the *observed biodiversity value* i.e. the
...
($\mu_{i}$) with some standard deviation ($\sigma_{residual}$) which we will set at 5:
...
for (i in seq_along(dat_list)) {
  ...
  y <- rnorm(n = replicates, mean = x$mu_i, sd = sigma_residual)
...
diamonds ($\mu_{i}$) and the simulated observed biodiversity values as circles ($B_i$).

```{r}
#| label: fig-5
#| fig-cap: "Simulated biodiversity values in the agricultural fields and in the food forests in the 30 sampled locations."
...
Using this simulated dataset, we can fit the linear mixed model using the [lme4 package](https://cran.r-project.org/web/packages/lme4/index.html) and see if we
...
from our fitted model. To extract the mean, we need the intercept coefficient `(Intercept)` in this model. In this
model, the estimated global mean (i.e. `(Intercept)`) was indeed very close to 100 (see below):
...
mean values match with simulated the mean values for each site and again, the model reproduces these values very well (the red, dashed line is the 1:1 line) (@fig-6).

```{r}
#| label: fig-6
#| fig-cap: "The relationship between the simulated (y-axis) and estimated (x-axis) base-level biodiversity values across the 30 locations."
...
better and better. But, now that we understand the assumptions and behaviour, we may ask ourselves are these
...
that the biodiversity difference between a food forest and an agricultural field might be very different. By
only fitting a single treatment effect parameter ($\beta_1$), the model is ignoring all of that variation and
...
locations. By doing this, we will obtain the average treatment effect across all 30 locations by
...
differs is that we are now fitting a separate intercept and a separate treatment effect for each location
rather than simply a separate intercept for each location ($\alpha_j$) and a single treatment effect ($\beta_1$) as we did in the first model. Moreover,
...
$$
B_{i} \sim Normal(\mu_{i}, \sigma_{\text{residual}})
$$
$$
\mu_{i} = \alpha_{j} + \beta_{j}(\text{treatment}_{i})
$$
$$
\begin{pmatrix}
\alpha_{j} \\
...
\end{pmatrix}
\right), \text{for location (L) } j = 1, \dots, J
$$

So, what does this model assume? It assumes that the control plots (i.e. the agricultural fields) at
all locations ($j$) have some base-level of biodiversity ($\alpha_j$). This is the same as the
...
some mean value of biodiversity ($\mu_{i}$). Importantly, the $\alpha_j$ and $\beta_j$ values come
from a common multivariate normal distribution. The observed biodiversity values ($B_{i}$) are
normally distributed around these mean values ($\mu_{i}$) for each location-group combination with
some residual standard deviation ($\sigma_{residual}$) as per the previous model.

How do we simulate this? To start with, we need to define the parameters of the multivariate normal distribution. As with the
...
```{r}
#| label: fig-7
#| fig-cap: "Multivariate normal distribution from which the base-level biodiversity values in the agricultural fields ($\alpha_j$) and the treatment effect, which is the difference between the food forest and the agricultural fields ($\beta_j$), are drawn."
...
dist_global <- MASS::mvrnorm(n = 1000000,
...
This figure shows 1000000 samples from the multivariate normal distribution that we set-up for
$\alpha_j$ and $\beta_j$ (@fig-7). As you can see, their values are correlated such that information from
$\alpha_j$ can provide information about $\beta_j$ (and *vice versa*). As this is the general distribution, we need to
...
```{r}
#| label: fig-8
#| fig-cap: "Multivariate normal distribution from which the base-level biodiversity values in the agricultural fields ($\alpha_j$) and the treatment effect, which is the difference between the food forest and the agricultural fields ($\beta_j$), are drawn where the red circles indicate the 30 sampled locations in our simulated study."
...
Using these sampled values of $\alpha_j$ and $\beta_j$ (@fig-8), we can now build a dataset with the means of
each control and treatment plot at each location ($\mu_{i}$). Let's visualise this.
...
dat_mu <- 
  dplyr::tibble(location = rep(c(seq_len(locations)), 2),
                treatment = rep(c("agricultural: control", "food-forest: treatment"),
                                each = locations),
                mu_i = c(dist_samp$a_j, (dist_samp$a_j + dist_samp$b_j)))
head(dat_mu)
...
(i.e. food forests) (@fig-9), the last thing we need to do is simulate the *observed biodiversity value* i.e.
...
Now we have a fully simulated dataset which we can plot and which shows the simulated mean values as
diamonds ($\mu_{i}$) and the simulated observed biodiversity values as circles ($B_i$) (@fig-10). As you can see, the difference between the biodiversity in food forests and agricultural fields differs between locations.
...
Finally, the model estimates a mean value for each location ($\alpha_j$) and a treatment effect for each location ($\beta_j$). We can then see how well the model was able to reproduce these specific values. In general, the model does reasonably well although the treatment effects ($\beta_j$) could be better (@fig-11).

```{r}
#| label: fig-11
#| fig-cap: "The relationship between the simulated (y-axis) and estimated (x-axis) base-level biodiversity values ($\alpha_j$) (left) and the betwee the simulated (y-axis) and estimated (x-axis) treatment effects ($\beta_j$) across the 30 locations. Diagonal line is the 1:1 line."
...
sim_lmm <- function(alpha_bar = 100, tau_alpha = 10, beta_bar = 5, tau_beta = 3, 
  ...
  dat <- 
    ...
    dplyr::rowwise() |>
    dplyr::mutate(
      B_i = list(rnorm(n = replicates, mean = mu_i, sd = sigma_residual))
    ) |> 
...
# simulate 1000 replicates of simulation 2
...
in model 1 (@fig-12). This is because the model is ignoring the variation among locations. Model 2 is less
...
```{r}
#| label: fig-12
#| fig-cap: "The difference between the t-values for the treatment effect in model 1 and model 2 when the data are simulated under the more realistic simulation 2 generating process."


# compare the t-values between the two simulations
ggplot(data = sim_rep_df |> dplyr::mutate(sim = ifelse(sim == "1", "Model 1", "Model 2")),
       mapping = aes(x = t_val, fill = sim)) +
...
variation to zero (`tau_beta = 0`). What happens in this case, is that the more complex model 2 becomes very similar
...
```{r}
#| label: fig-13
#| fig-cap: "The difference between the t-values for the treatment effect in model 1 and model 2 when the data are simulated under the model 1 data-generating process with no variation among treatment effects."
...
difference is that the observed biodiversity ($B_{i}$) is Poisson distributed around some
($\lambda_{i}$). Moreover, the $\alpha_{j}$ and $\beta_{j}$ parameters determine the
$log(\lambda_{i}$ value. This ensures that the $\lambda_{i}$ values are positive and that the
linear model goes from negative infinity to positive infinity.

$$
B_{i} \sim \text{Poisson}(\lambda_{i})
$$
$$
\log(\lambda_{i}) = \alpha_{j} + \beta_{j}(treatment_{i})
$$
$$
\begin{pmatrix}
\alpha_{j} \\
...
exponentiated $\mu_{i}$ value and pass it through the Poisson distribution to generate the observed
biodiversity values ($B_{i}$).
...
dat <- 
  ...
  dplyr::rowwise() |>
  dplyr::mutate(
    B_i = list(rpois(n = replicates, lambda = exp(mu_i)))
    ) |> 
  dplyr::select(location, treatment, B_i) |>
  tidyr::unnest(cols = c(B_i))
